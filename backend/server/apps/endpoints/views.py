	...
	def perform_create(self, serializer):
		try:
			with transaction.atomic():
				...
				# set active=False for other status
...
class ABTestViewSet(mixins.RetrieveModelMixin, mixins.ListModelMixin, mixins.CreateModelMixin, 
	...
	def post(self, request, ab_test_id, format=None):
		try:
			...
			if ab_test.ended_at is not None:
				return Response({"message": "A/B test already finished."})

			date_now = datetime.datetime.now()

			# alg #1 accuracy
			al_response_1 = MLRequest.objects.filter(
			...
			accuracy_1 = correct_response_1 / float(al_response_1)
			print(al_response_1, correct_response_1, accuracy_1)

			# alg #2 accuracy
			al_response_2 = MLRequest.objects.filter(
			...
			accuracy_2 = correct_response_2 / float(al_response_2)
			print(al_response_2, correct_response_2, accuracy_2)
			...
			sumary = "Algorithn #1 accuracy: {}, Algorithm #2 accuracy: {}".format(accuracy_1, accuracy_2)
			ab_test.ended = date_now
			ab_test.sumary = sumary
			ab_test.save()
